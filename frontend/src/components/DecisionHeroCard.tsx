  ...
  const getCardTheme = () => {
    if (isRiskOverride || rec === 'SELL_EARLY_DUE_TO_RISK') {
      return {
        ...
        actionTag: '⚠️ RISK OVERRIDE',
        actionTitle: `Sell Early at ${decision.recommendedMandi?.mandiName || topMandi?.mandi.mandiName || 'Nearest Mandi'}`,
    ...
    if (rec === 'SELL_AT_RECOMMENDED_MANDI' || rec === 'TRAVEL') {
      return {
        ...
        actionTag: '✨ RECOMMENDED MARKET',
        actionTitle: `Sell at ${decision.recommendedMandi?.mandiName || topMandi?.mandi.mandiName || 'Recommended Mandi'}`,
    ...
    if (rec === 'HOLD') {
      return {
        ...
        actionTag: '⏳ STRATEGIC HOLD',
        actionTitle: `Hold ${commodityName} for Expected Peak Price`,
    ...
    return {
      ...
      actionTag: '📦 SELL NOW',
      actionTitle: `Sell Now at ${decision.recommendedMandi?.mandiName || topMandi?.mandi.mandiName || 'Local APMC'}`,
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isRiskOverride && (
              <span className="px-2.5 py-0.5 rounded-full font-semibold bg-amber-500/25 text-amber-200 border border-amber-500/40 flex items-center gap-1">
                ...
                Base Decision (HOLD) Overridden for Safety
              </span>
            )}
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="p-3.5 bg-emerald-500/20 border border-emerald-400/40 rounded-2xl">
                ...
                <span className="text-[11px] text-emerald-200/80 mt-0.5 block">
                  (Estimated net after transit & risk protection)
                </span>
              </div>
              ...
              <div className="p-3.5 bg-white/5 border border-white/10 rounded-2xl flex flex-col justify-between">
                ...
                <div className="mt-1">
                  ...
                  <span className="text-[11px] text-slate-400">
                    Expected Peak Price (+{(((forecast.expectedPeakPrice - forecast.currentPrice) / Math.max(forecast.currentPrice, 1)) * 100).toFixed(1)}%)
                  </span>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-white/5 border border-white/10 rounded-2xl flex flex-col justify-between">
                <div className="flex items-center justify-between text-xs text-slate-300">
                  ...
                  <span className="font-semibold text-slate-300">{topMandi.distanceKm} km away</span>
                </div>
                <div className="mt-1">
                  ...
                  <span className="text-[11px] text-slate-400">
                    ₹{topMandi.expectedRevenue.toLocaleString('en-IN')} gross - ₹{topMandi.totalTransportCost.toLocaleString('en-IN')} transit
                  </span>
                </div>
              </div>
            </div>
